...
_THEME_SYNC_JS_PATH = _STATIC_DIR / "js" / "theme_iframe_sync.js"
_clipboard_iframe_assets_cache: tuple[str, str, str] | None = None


def clipboard_iframe_assets() -> tuple[str, str, str]:
    ...
    if _clipboard_iframe_assets_cache is None:
        _clipboard_iframe_assets_cache = (
            ...
            _THEME_SYNC_JS_PATH.read_text(encoding="utf-8"),
        )
...
def render_clipboard_iframe_button(
    *,
    ...
    if not value:
        return
    css, js, theme_sync_js = clipboard_iframe_assets()
    ...
    body = f"""{header}<script>{theme_sync_js}</script>
<style>{css}</style>
