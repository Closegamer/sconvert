...
FREQUENCY_UNITS: list[tuple[str, str, float]] = [
    ("rpm", "units.frequency.rpm", 1.0 / 60.0),
    ("hz", "units.frequency.hz", 1.0),
    ("khz", "units.frequency.khz", 1_000.0),
    ("mhz", "units.frequency.mhz", 1_000_000.0),
    ("ghz", "units.frequency.ghz", 1_000_000_000.0),
]
...
def render_frequency_converter(texts: dict[str, str]) -> None:
    if "units_frequency_base_hz" not in st.session_state:
        st.session_state.units_frequency_base_hz = 1.0
    if "units_frequency_last_inputs" not in st.session_state:
        st.session_state.units_frequency_last_inputs = {}

    input_keys = [f"units_frequency_{code}" for code, _k, _f in FREQUENCY_UNITS]
    needs_sync = any(key not in st.session_state for key in input_keys)

    if not needs_sync:
        last_inputs: dict[str, str] = st.session_state.units_frequency_last_inputs
        current_inputs = {
    ...
    favorite_toggle_key = "favorite_frequency_toggle"
    if favorite_toggle_key not in st.session_state:
        st.session_state[favorite_toggle_key] = bool(st.session_state.favorite_frequency)

    collapse_token = int(st.session_state.get("units_collapse_all_token", 0))
    seen_token_key = "units_frequency_seen_collapse_token"
    expanded_key = "units_frequency_expanded"
    ...
    render_unit_panel_header(
        panel_key="units_frequency_panel",
        title=texts["units.frequency.title"],
        expanded_key=expanded_key,
        expand_button_key="units_frequency_expand_btn",
        favorite_toggle_key=favorite_toggle_key,
        favorite_state_key="favorite_frequency",
    )

    if st.session_state[expanded_key]:
        left_col, right_col = st.columns(2, gap="small")
        columns = [left_col, right_col]
        for idx, (unit_code, label_key, _factor) in enumerate(FREQUENCY_UNITS):
            with columns[idx % 2]:
                st.text_input(
                    texts[label_key],
                    key=f"units_frequency_{unit_code}",
                )
    st.session_state.units_frequency_last_inputs = {
        code: st.session_state.get(f"units_frequency_{code}", "") for code, _k, _f in FREQUENCY_UNITS
    }
